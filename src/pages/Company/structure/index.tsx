  ...
  return data?.map((i: any, index: number) => {
    ...
    if (level + 1 == 4) {
      return {
        ...
        ['positionlevel' + (level - 1) + '_id']:
          masid !== '' ? masid : i?.data?.id,
      };
    } else if (level == 1) {
      return {
        ['id_Position' + level]: i?.data?.id,
...
const CompanyStructure: React.FC = () => {
  ...
  let props = location.state as any;
  console.log(props);
  ...
  const DELEATEPOSITION1 =
    gql(`mutation Delete_position1($deletePosition1Id: ID!) {
        delete_position1(id: $deletePosition1Id) {
          message
          status
        }
      }`);
  const DELEATEPOSITION2 =
    gql(`mutation Delete_position2($deletePosition2Id: ID!) {
        delete_position2(id: $deletePosition2Id) {
          message
          status
        }
      }`);
  const DELEATEPOSITION3 =
    gql(`mutation Delete_position3($deletePosition3Id: ID!) {
      ...
      <Row style={{ height: '100%' }}>
        ...
        <Col span={16} style={{ padding: '0px 10px' }}>
          ...
          <Tooltip title="รีเซ็ต">
            ...
            >
              {' '}
              <AiOutlineSync size={25} color="white" />
            </ButtonReset>
          </Tooltip>
          <StyledTree
            className="draggable-tree"
